...
FDFilterDialog::FDFilterDialog(QImage inputImage)
{
    ...
    // spectrum QImage  不要被名字迷惑了  其实就是将频谱向量数据转化成一幅图片
    ...
    filteredSpectrumImageLabel->setPixmap(QPixmap::fromImage(filteredSpectrumImage).scaled(filteredSpectrumImageLabel->width(), filteredSpectrumImageLabel->height()));
    dstImageLabel->setPixmap(QPixmap::fromImage(dstImage).scaled(dstImageLabel->width(), dstImageLabel->height()));
}
...
void FDFilterDialog::iniUI()
{
    ...
    spectrumImageLabel->setScaledContents(true);
    filteredSpectrumImageLabel->setScaledContents(true);
    ...
    // signal slot  当参数或类型发生变化时 及时更新图片
