...
//这个函数为 下面的  两个中间函数服务  而下面的两个中间函数又为  最底下的 copyMakeBorder 服务
...
//是上面两个 边界处理函数的集成版本
//在 SDFilterDialog::imageLOGFilter  中将会调用此函数进行边界扩充
//部分参数含义：
//        ...
//        cn  通道数
//        top  图像上方的扩充数
...
//      value  当为指定值插值时需要给定具体值 value 从而利用它进行边界扩充
//     注意value 的长度与通道数一致  分别代表不同通道的常数扩充大小
