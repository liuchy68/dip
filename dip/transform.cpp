...
void fftshift2D(complex<float> *src, int w, int h, complex<float> *dst)
{
    ...
    for(i = 0; i < h; i++)
    {
        //3 * w表示沿水平方向拼接图像的三个通道向量
        for(j = 0; j < 3 * w; j++)
        {

            //回想一下fft的潜在周期性就能理解下面代码的用意了
            if (i < h / 2)
                ii = i + h / 2;
            else
                ii = i - h / 2;
...
void calcImageSpectrum(QImage src, QImage &dst)
{
    ...
    //注意channels指向的是 一个长度为 n*3 的一维数组
    ...
    //分别将原图像三通道数据填入rr  gg  bb 相对应的一维数组中
    //之后便能通过 访问channels所指向的数组 而 间接得到原图的各个数据点了
    splitImageChannel(src, rr, gg, bb);

    //创建两个复数指针
    fftwf_complex *y, *temp;

    y         =  (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * 3*n);  //用于保存fft后的结果
    temp  =  (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * 3*n);  //用于保存进行fftshift后的结果
